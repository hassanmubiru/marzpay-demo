...
function renderSuccess(fields) {
    ...
    return template.replace(/\{\{(reference|amount|currency|status|statusMessage)\}\}/g, (token) => replacements[token] ?? "");
}
/**
...
let SuccessController = class SuccessController {
    ...
    async show(ctx) {
        ...
        if (reference === undefined || reference.trim() === "") {
            ctx.html(errorPage("Reference Required", REFERENCE_REQUIRED_MESSAGE), 400);
            return;
        }
...
], SuccessController.prototype, "show", null);
