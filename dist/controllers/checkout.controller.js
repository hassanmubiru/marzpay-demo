...
let CheckoutController = class CheckoutController {
    ...
    async create(ctx) {
        ...
        ctx.setHeader("Location", `/success?reference=${encodeURIComponent(reference)}`);
        ctx.send(303);
    }
};
__decorate([
 ...
 */
async function readPhoneNumber(ctx) {
    const body = ctx.body;
    if (body !== null && typeof body === "object" && !Array.isArray(body)) {
        const value = body["phone_number"];
...
function phoneFromUrlEncoded(raw) {
...
function phoneFromJson(raw) {
    try {
        ...
        if (parsed !== null && typeof parsed === "object" && !Array.isArray(parsed)) {
            const value = parsed["phone_number"];
