...
import { verifyManifest, PluginError, PluginSignatureError, PluginStateError, PluginManifestError, } from './host.js';
...
const KNOWN_PERMISSIONS = ['middleware', 'events', 'net', 'fs', 'db', 'secrets'];
/**
 ...
 */
export function assertWellFormedManifest(manifest) {
    ...
    if (m.permissions !== undefined
        && (!Array.isArray(m.permissions) || m.permissions.some((p) => !KNOWN_PERMISSIONS.includes(p)))) {
 ...
 */
export async function installThroughRegistry(registry, host, plugin, opts = {}) {
    if (!host.verifiesSignatures()) {
        throw new PluginError('Refusing to install through the registry: the PluginHost must be configured with a trusted public key to enforce signature verification');
    }
    ...
    if (durationMs > timeoutMs) {
        throw new PluginError(`Plugin "${plugin.name}@${plugin.version}" install exceeded the ${timeoutMs}ms budget (took ${durationMs}ms)`);
    }
