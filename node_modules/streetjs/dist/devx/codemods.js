 ...
 */
function findParseObstruction(src) {
    const n = src.length;
    const stack = [];
    let i = 0;
    while (i < n) {
    ...
    if (stack.length > 0) {
        ...
        if (top === 'interp')
            return 'unterminated template-literal interpolation';
 ...
 */
export function safeRenameCodemod(id, from, to, area, description) {
    const re = new RegExp(`\\b${escapeRe(from)}\\b`, 'g');
    return {
        id,
        description,
        ...
        apply(source) {
            ...
            if (!hasFrom)
                return { code: source, changed: false, changes: 0 };
...
export const ROUTING_CODEMODS = [
    safeRenameCodemod('rename-router-context', 'RouterContext', 'RouteContext', 'routing', 'Rename the routing handler context type RouterContext to the canonical RouteContext.'),
    safeRenameCodemod('rename-route-handler-type', 'RouteHandlerFn', 'RouteHandler', 'routing', 'Rename the deprecated RouteHandlerFn type alias to RouteHandler.'),
];
/** Middleware-area migrations. */
export const MIDDLEWARE_CODEMODS = [
    safeRenameCodemod('rename-middleware-next', 'MiddlewareNext', 'NextFunction', 'middleware', 'Rename the MiddlewareNext callback type to the canonical NextFunction.'),
    safeRenameCodemod('rename-use-middleware', 'useMiddleware', 'use', 'middleware', 'Rename the deprecated app.useMiddleware registration method to app.use.'),
];
/** Plugin-API-area migrations. */
export const PLUGIN_API_CODEMODS = [
    safeRenameCodemod('rename-plugin-register', 'registerPlugin', 'usePlugin', 'plugin-api', 'Rename the deprecated registerPlugin host method to usePlugin.'),
    safeRenameCodemod('rename-plugin-context', 'PluginContext', 'PluginHost', 'plugin-api', 'Rename the PluginContext type to the canonical PluginHost.'),
];
