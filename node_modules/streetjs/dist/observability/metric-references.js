...
import { Histogram } from './prometheus.js';
 ...
 */
export function exportedMetricNames(registry) {
    const out = new Set();
 ...
 */
export function referencedMetrics(assets) {
    const out = new Set();
    for (const dashboard of assets.dashboards ?? []) {
        for (const panel of dashboard.panels ?? []) {
            for (const target of panel.targets ?? []) {
                for (const metric of extractMetricsFromExpr(target.expr))
                    out.add(metric);
            }
        }
    }
    for (const group of assets.rules ?? []) {
        for (const rule of group.rules ?? []) {
            for (const metric of extractMetricsFromExpr(rule.expr))
                out.add(metric);
        }
    }
    return out;
 ...
 * panels or rules.
 */
export function validateMetricReferences(exported, assets) {
    const violations = [];
    const seen = new Set();
    const record = (metric, asset) => {
        if (exported.has(metric))
            return;
...
//# sourceMappingURL=metric-references.js.map