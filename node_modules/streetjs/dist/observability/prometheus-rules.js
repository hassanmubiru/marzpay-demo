 ...
 */
export function streetSubsystemAlertRules() {
    return {
 ...
 */
export function streetSloObjectives() {
    return [
...
export function streetSloPack() {
    ...
    const availExpr = `(1 - (sum(rate(http_requests_total{status=~"5.."}[${w}])) / sum(rate(http_requests_total[${w}])))) < ${availability.target}`;
    const latencyExpr = `histogram_quantile(0.99, sum(rate(http_request_duration_seconds_bucket[${latency.window}])) by (le)) > ${latency.target}`;
    return {
