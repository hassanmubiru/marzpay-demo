 ...
 */
export const PLATFORM_LEADERSHIP_CAPABILITIES = Object.freeze([
    'dast.scan',
    'cloud.deploy',
 ...
 */
const MISSING_ARTIFACT_STATUS = 'NOT_IMPLEMENTED';
/**
 ...
 */
function resolveRollup(capabilityId, artifacts) {
    const rollup = ROLLUPS.find((r) => r.id === capabilityId);
    if (!rollup)
        return null;
    const members = [];
    for (const [id, artifact] of artifacts) {
        if (rollup.isMember(id))
    ...
    const status = ROLLUP_STATUS_SEVERITY.find((s) => s !== 'VERIFIED' && members.includes(s)) ??
        'PARTIAL';
 ...
 */
export function computeLeadership(artifacts, now = new Date()) {
