...
function coreTestCommand(testFiles) {
...
export function defaultCapabilityPlans(opts) {
    ...
    const dating = (capabilityId, pkgDir) => {
    ...
    const plans = {
        'validation.runtime': core('validation.runtime', [
            'validation.test.js',
        ...
        'ratelimit.sliding-window': core('ratelimit.sliding-window', [
            'ratelimit-sliding-window-pbt.test.js',
            'ratelimit-window-parsing-pbt.test.js',
            'ratelimit-scopes.test.js',
            'store.test.js',
        ], 'security/ratelimit.ts'),
        'headers.defaults': core('headers.defaults', ['headers-set-invariance-pbt.test.js', 'security-headers-defaults.test.js'], 'security/headers.ts'),
        'upload.guard': core('upload.guard', [
        ...
        'encryption.field': core('encryption.field', [
        ...
        'secrets.provider': core('secrets.provider', ['secret-provider-adapters.test.js'], 'security/secret-provider.ts'),
        'privacy.controls': core('privacy.controls', [
    ...
    return CONSUMER_PLATFORM_CAPABILITIES.filter((id) => id in plans).map((id) => plans[id]);
}
/** Resolve the artifact output directory for a capability: `<outRoot>/<area>/`. */
function outDirFor(repoRoot, outRoot, capabilityId) {
...
export async function captureEvidence(opts) {
    ...
    for (const plan of selected) {
        ...
        logger.log(`[capture]   ${marker} ${artifact.status} (exit ${artifact.exitCode}) → ${path}`);
        if (artifact.blockedReason) {
            logger.log(`[capture]     blocked: ${artifact.blockedReason.kind}/${artifact.blockedReason.missingPrerequisite}`);
        }
        results.push({
            capabilityId: plan.capabilityId,
...
//# sourceMappingURL=capture-evidence.js.map