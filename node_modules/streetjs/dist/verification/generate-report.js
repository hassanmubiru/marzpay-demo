...
import { computeCertification, } from './certification.js';
...
export function collectArtifactFiles(rootDir) {
    ...
    for (const entry of entries) {
        if (!entry.isFile())
            continue;
        if (!entry.name.endsWith(ARTIFACT_SUFFIX))
            continue;
 ...
 */
export function loadArtifactSources(rootDir, logger = console) {
    const sources = [];
    for (const filePath of collectArtifactFiles(rootDir)) {
        ...
        try {
            ...
            logger.error(`[report]   skipping unreadable artifact ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
            continue;
        }
        ...
        sources.push({ artifact: parsed, path: filePath });
    }
    return sources;
}
/**
 ...
 */
export function renderHumanReadable(report, sources) {
    // Map each capability to the artifact path that evidences it (R12.4).
    const evidencePath = new Map();
    ...
    lines.push('');
    lines.push('─'.repeat(72));
    lines.push(certifiedCount === report.categories.length
    ...
    try {
        ...
        try {
            if (existsSync(tmpPath))
                renameSync(tmpPath, `${tmpPath}.orphan`);
        }
        catch {
...
export function generateReport(opts) {
    ...
    if (!existsSync(rootDir)) {
        throw new Error(`Certification Report: artifact directory not found: ${rootDir}. ` +
...
function findRepoRoot(startDir) {
    let dir = startDir;
    for (let i = 0; i < 8; i++) {
        if (existsSync(join(dir, 'packages')) && existsSync(join(dir, '.git')))
            return dir;
...
async function main() {
    ...
    if (textPath)
        console.log(`[report] Text written to: ${textPath}`);
