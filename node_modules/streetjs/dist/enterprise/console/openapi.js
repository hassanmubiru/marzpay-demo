...
const SPEC_DESCRIPTION = 'REST surface for tenant, policy, compliance, and administrative operations. ' +
...
function toRouteInput(route) {
    ...
    return {
        ...
        description: `${route.summary}. Requires one of the roles: ${route.requiredRoles.join(', ')}.`,
        tags: [route.area],
        responses,
    };
}
 ...
 */
export function consoleOpenApiSpec(routes = CONSOLE_ROUTES) {
    const base = generateOpenApi(routes.map(toRouteInput));
    ...
    for (const route of routes) {
        ...
        const op = base.paths[openApiPath]?.[method];
        if (!op)
            continue;
