 ...
 */
export function generateEcsService(config) {
...
export function validateEcsService(manifest) {
    const errors = [];
    const need = (cond, msg) => { if (!cond)
        errors.push(msg); };
    let parsed = null;
...
function smokeTestScript(target, cfg) {
...
function lambdaColdStartValidation(cfg) {
    return `#!/usr/bin/env node
...
// ── Azure Functions ─────────────────────────────────────────────────────────────
function azureFunctionsAssets(cfg) {
    return {
...
// ── Cloudflare Workers ──────────────────────────────────────────────────────────
function cloudflareWorkersAssets(cfg) {
    return {
...
`;
function cloudflareValidation(cfg) {
    return `#!/usr/bin/env node
...
function cloudflareWorkflow(cfg) {
 ...
 */
export function classifyTargetVerification(tv) {
    ...
    if (!tv.health.live)
        violations.push('/health/live did not report healthy');
    if (!tv.health.ready)
        violations.push('/health/ready did not report healthy');
    if (tv.health.maxLatencyMs > HEALTH_LATENCY_BUDGET_MS) {
        violations.push(`health latency ${tv.health.maxLatencyMs}ms exceeded the ${HEALTH_LATENCY_BUDGET_MS}ms budget`);
    }
    ...
    if (tv.smoke === undefined) {
        ...
        if (tv.smoke.failed > 0)
            violations.push(`${tv.smoke.failed} smoke case(s) failed`);
        if (tv.smoke.errored > 0)
            violations.push(`${tv.smoke.errored} smoke case(s) errored`);
        if (tv.smoke.durationMs > SMOKE_DURATION_BUDGET_MS) {
            violations.push(`smoke duration ${tv.smoke.durationMs}ms exceeded the ${SMOKE_DURATION_BUDGET_MS}ms budget`);
        }
    }
    if (violations.length === 0) {
        // All bounds satisfied ⇒ VERIFIED; clear any stale violations.
        const verified = { ...tv, status: 'VERIFIED' };
 ...
 */
export function buildDeploymentReport(results) {
    return {
 ...
 */
export function targetDependencies(target) {
    switch (target) {
        case 'kubernetes':
            return [
        ...
        default:
            throw new Error(`targetDependencies: unknown target "${target}"`);
    }
}
/**
 ...
 */
export function lintWorkflow(yaml) {
    const errors = [];
    const need = (cond, msg) => { if (!cond)
        errors.push(msg); };
...
function presenceCheck(name, content, mustInclude) {
    const errors = [];
...
function jsonCheck(name, content) {
    if (typeof content !== 'string' || content.trim() === '') {
...
function helmChartStructureChecks(assets) {
    ...
    if (typeof chart !== 'string' || chart.trim() === '')
        chartErrors.push('Chart.yaml missing');
    else {
        if (!/^name:\s*\S+/m.test(chart))
            chartErrors.push("Chart.yaml missing 'name:'");
        if (!/^version:\s*\S+/m.test(chart))
            chartErrors.push("Chart.yaml missing 'version:'");
    }
 ...
 */
export function runOfflineArtifacts(target, cfg) {
    ...
    switch (target) {
        ...
        case 'cloudflare-workers': {
            ...
            if (typeof toml !== 'string' || toml.trim() === '')
                tomlErrors.push('wrangler.toml missing');
            else {
                if (!/main\s*=\s*"\S+"/.test(toml))
                    tomlErrors.push("wrangler.toml missing 'main'");
                if (!/compatibility_date\s*=/.test(toml))
                    tomlErrors.push("wrangler.toml missing 'compatibility_date'");
            }
        ...
        default:
            throw new Error(`runOfflineArtifacts: unknown target "${target}"`);
    }
 ...
 */
export function blockedTargetWithOfflineEvidence(target, missing, offline) {
    return {
        target,
        status: 'BLOCKED',
