...
export class UploadGuard {
    ...
    async guard(file) {
        ...
        if (file.size > this.policy.maxBytes) {
            ...
            throw new UploadRejected('TOO_LARGE', `File exceeds maximum size of ${this.policy.maxBytes} bytes`);
        }
        let contents;
        try {
            ...
            await this.discard(file);
            throw new UploadRejected('MIME_MISMATCH', `Unable to read uploaded file: ${err.message}`);
        }
        ...
        if (this.policy.imageOnly) {
            if (detected === null || !IMAGE_MIME_TYPES.has(detected.mime)) {
                await this.discard(file);
                throw new UploadRejected('DISALLOWED_TYPE', 'Only image uploads are permitted');
            }
        }
        ...
        if (detected === null) {
            await this.discard(file);
            throw new UploadRejected('MIME_MISMATCH', 'Could not determine the true file format from its contents');
        }
        if (detected.mime !== file.mimeType) {
            await this.discard(file);
            throw new UploadRejected('MIME_MISMATCH', `Declared MIME type "${file.mimeType}" does not match detected type "${detected.mime}"`);
        }
        ...
        if (this.policy.allowedMimeTypes && !this.policy.allowedMimeTypes.includes(detected.mime)) {
            await this.discard(file);
            throw new UploadRejected('DISALLOWED_TYPE', `MIME type "${detected.mime}" is not allowed`);
        }
        ...
        if (this.policy.malwareScan) {
            ...
            try {
                ...
                await this.discard(file);
                throw new UploadRejected('MALWARE', `Malware scan failed: ${err.message}`);
            }
            if (verdict.malicious) {
                await this.discard(file);
                throw new UploadRejected('MALWARE', verdict.reason ?? 'File flagged as malicious by malware scan');
            }
        }
